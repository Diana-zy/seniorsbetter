...
export function generateCustomLink(url) {
  ...
  if (match) {
    ...
    if (!prefix) targetDomain = `www.${currentDomain}`;
    else if (prefix.startsWith("www")) {
      if (prefix === "www") targetDomain = currentDomain.substring(4);
      else targetDomain = currentDomain.substring(3);
    } else {
  ...
  return `${protocol}//${targetDomain}${targetPath}?${queryString}${queryString ? "&" : ""}from=${encodeURIComponent(from)}`;
}
